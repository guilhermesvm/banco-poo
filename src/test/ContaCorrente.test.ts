...
describe("Testa as funcionalidades de uma Conta Corrente", () => {
    ...
    test("Deve criar uma conta corrente válida", () => {
        const contaCorrente = new ContaCorrente(numero, agencia, saldo, limite);
    ...
    test("Deve usar o cheque especial corretamente quando o saldo é insuficiente", () => {
      ...
      test("Não deve definir um limite de cheque especial negativo", () => {
        expect(() => new ContaCorrente("12345-6", "0001-0", 1000, -500)).toThrow("O limite do cheque especial deve ser um valor não negativo.");
      });

      test("Deve retornar o tipo da conta", () => {
        const contaCorrente = new ContaCorrente(numero, agencia, saldo, limite);
        expect(contaCorrente.getTipo).toBe("Corrente");
      });
})